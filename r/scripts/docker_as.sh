...
DOCKER_CMD="docker run -it --rm -v $(pwd):${DOCKER_MOUNTPOINT}:rw --user $DOCKER_USER --env __VERBOSITY=${__VERBOSITY:-} --env __DISTRO=${__DISTRO:-} --env __MINIMAL_TESTS=${__MINIMAL_TESTS:-} $DOCKER_IMAGE $DOCKER_COMMAND_ARGS"
>&2 echo $DOCKER_CMD
$DOCKER_CMD
