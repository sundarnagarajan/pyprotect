...
BUILD_SCRIPT=${DOCKER_MOUNTPOINT}/${SCRIPTS_DIR}/inplace_build.sh
...
function build_1_pyver() {
    ...
    cd "$PROG_DIR"/..
    "${CLEAN_BUILD_SCRIPT}"
    DOCKER_CMD="docker run --rm -v $(pwd):${DOCKER_MOUNTPOINT}:rw --user "${HOST_UID}:${HOST_GID}" --env __DISTRO=${__DISTRO:-} $img ${BUILD_SCRIPT} $pyver"
    $DOCKER_CMD
    "${CLEAN_BUILD_SCRIPT}"
}
