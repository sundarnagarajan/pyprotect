#!/bin/bash
# As of now  Arch has Cython3, PY2, PY3, PYPY2, PYPY3 by using AUR
...
[[ -n $(declare -p __CONFIG_SOURCED 2>/dev/null) && -n "${__CONFIG_DOCKER_SOURCED}+_" ]] || {
    ...
    COMMON_CONTAINER_NAME=py23_pypy23:archlinux
    # If $CYTHON_DOCKER_FILE is in DOCKERFILE_IMAGE, CYTHON3_DOCKER_IMAGE
    # is not required
    CYTHON_DOCKER_FILE=Dockerfile.arch
